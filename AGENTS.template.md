...
2. Discuss or identify key takeaways and emphasis.
3. Append source integration sections without changing raw source content.
4. Create or update wiki pages with citations.
5. If a new claim contradicts an existing claim, add a contradictions section rather than overwriting silently.
6. Add source backlinks.
7. Create tag stubs as needed.
8. Update the canonical index.
9. Append to the operations log.
10. Bump `updated:` on every typed page touched.
...
Output a checklist report. Propose structural fixes before applying them.
